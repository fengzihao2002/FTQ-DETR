...
class DeformableTransformer(nn.Module):
    def __init__(
        ...
        self.decoder = DeformableTransformerDecoder(
            ...
            use_checkpoint,
            use_tddfd=use_tddfd,
            tddfd_start_layer=tddfd_start_layer,
        )
    ...
    def forward(self, srcs, masks, pos_embeds, query_embed=None, self_attn_mask=None):
        ...
        return hs_cls, hs_loc, init_reference_out, inter_references_out, None, None
...
class DeformableTransformerDecoderLayer(nn.Module):
    def __init__(
        ...
        self.use_dual_field = False

        # shared cross attention for the original decoder stage
        self.cross_attn = MSDeformAttn(d_model, n_levels, n_heads, n_points)
        self.dropout1 = nn.Dropout(dropout)
        self.norm1 = nn.LayerNorm(d_model)

        # task-decoupled cross attention used in the latter decoder stages
        ...
        # self attention stays shared
        self.self_attn = nn.MultiheadAttention(d_model, n_heads, dropout=dropout)
        self.dropout2 = nn.Dropout(dropout)
        self.norm2 = nn.LayerNorm(d_model)

        # shared ffn for the original decoder stage
        self.linear1 = nn.Linear(d_model, d_ffn)
        self.activation = _get_activation_fn(activation)
        self.dropout3 = nn.Dropout(dropout)
        self.linear2 = nn.Linear(d_ffn, d_model)
        ...
        # branch-specific ffn for task decoupling stages
    ...
    def forward_ffn_branch(self, tgt, linear1, dropout3, linear2, dropout4, norm3):
        tgt2 = linear2(dropout3(self.activation(linear1(tgt))))
    ...
    def forward(
        ...
        if not self.use_dual_field:
            # original shared cross attention in the early decoder layers
        ...
        # task-decoupled dual-field cross attention in the later decoder layers
        tgt_cls = self.cross_attn_cls(
            self.with_pos_embed(tgt, query_pos),
            reference_points,
            src,
            src_spatial_shapes,
        ...
        tgt_cls = tgt + self.dropout1_cls(tgt_cls)
        tgt_cls = self.norm1_cls(tgt_cls)
        tgt_cls = self.forward_ffn_branch(
            tgt_cls,
            self.linear1_cls,
            self.dropout3_cls,
            self.linear2_cls,
        ...
        tgt_loc = tgt + self.dropout1_loc(tgt_loc)
        tgt_loc = self.norm1_loc(tgt_loc)
        tgt_loc = self.forward_ffn_branch(
            tgt_loc,
            self.linear1_loc,
            self.dropout3_loc,
            self.linear2_loc,
        ...
        # A1 keeps the next decoder input shared via simple averaging only.
        tgt_next = 0.5 * (tgt_cls + tgt_loc)
        return tgt_next, tgt_cls, tgt_loc
...
class DeformableTransformerDecoder(nn.Module):
    def __init__(
        ...
        self.use_tddfd = bool(use_tddfd)
        self.tddfd_start_layer = int(tddfd_start_layer)
        for lid, layer in enumerate(self.layers):
            layer.use_dual_field = self.use_tddfd and lid >= self.tddfd_start_layer
    ...
    def forward(
        ...
        for lid, layer in enumerate(self.layers):
            ...
            if self.use_checkpoint:
                output, output_cls, output_loc = checkpoint.checkpoint(
                    layer,
                    output,
                    query_pos,
                    reference_points_input,
                ...
                output, output_cls, output_loc = layer(
                    output,
                    query_pos,
                    reference_points_input,
                    src,
            ...
            # iterative bounding box refinement follows the localization branch
            if self.bbox_embed is not None:
                tmp = self.bbox_embed[lid](output_loc)
                if reference_points.shape[-1] == 4:
        ...
        if self.return_intermediate:
            return (
                torch.stack(intermediate_cls),
                torch.stack(intermediate_loc),
                torch.stack(intermediate_reference_points),
            )

        return output_cls, output_loc, reference_points
...
def build_deforamble_transformer(args):
    return DeformableTransformer(
        ...
        use_tddfd=args.use_tddfd,
        tddfd_start_layer=args.tddfd_start_layer,
    )
