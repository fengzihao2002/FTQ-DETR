...
def train_one_epoch(
    ...
    model.train()
    criterion.train()
    # FGBC uses current epoch for residual warm-up when enabled.
    if hasattr(model, "module") and hasattr(model.module, "set_current_epoch"):
        model.module.set_current_epoch(epoch)
    elif hasattr(model, "set_current_epoch"):
...
def evaluate(
    ...
    # Compatible with single-GPU and DDP execution
    base_model = model.module if hasattr(model, "module") else model

    # disable the one-to-many branch queries
    # save them first
    save_num_queries = base_model.num_queries
    save_two_stage_num_proposals = base_model.transformer.two_stage_num_proposals
    base_model.num_queries = base_model.num_queries_one2one
    base_model.transformer.two_stage_num_proposals = base_model.num_queries
    ...
    base_model.num_queries = save_num_queries
    base_model.transformer.two_stage_num_proposals = save_two_stage_num_proposals
